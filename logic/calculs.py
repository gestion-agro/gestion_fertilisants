...
def calculer_doses(window):
    ...
    # --- Choix du mode ---
    if not ferts:
        debug.debug("➡️ Aucun fertilisant manuel → mode AUTO forcé")
        resultats = calcul_auto(window, Nb, Pb, Kb)
    else:
        dlg = ChoixModeCalcul(window)
        if dlg.exec() != QDialog.Accepted:
            debug.debug("❌ Choix du mode annulé")
            return

        mode = dlg.mode
        debug.debug("Mode choisi :", mode)
...
def calcul_auto(window, Nb, Pb, Kb):
    ...
    for row in range(window.table_fertilisants.rowCount()):
        ...
        if not cell_widget:
            debug.debug(f"⛔ Pas de widget checkbox ligne {row}")
            continue

        chk = cell_widget.layout().itemAt(0).widget()
        nom = window.table_fertilisants.item(row, 0).text()

        debug.debug(f"Ligne {row} → {nom} | checked={chk.isChecked()}")

        if chk.isChecked():
            fert = next((f for f in window.fert_base if f["nom"] == nom), None)
            if not fert:
                debug.debug("⛔ Fertilisant absent de fert_base :", nom)
                continue
    ...
    if not window.fertilisants_autorises:
        debug.debug("❌ Aucun fertilisant autorisé → abandon")
        return {"fertilisants": []}
    ...
    debug.debug("Variables x :", list(x.keys()))
    debug.debug("Variables y :", list(y.keys()))
    ...
    debug.debug("Contraintes NPK posées")

    for f in window.fertilisants_autorises:
        prob += y[f["nom"]] <= M * x[f["nom"]]
        debug.debug(f"Lien x/y :", f["nom"])
    ...
    for f in window.fertilisants_autorises:
        dose = y[f["nom"]].value()
        debug.debug(f"Résultat {f['nom']} → dose = {dose}")
    ...
    debug.debug("Fertilisants FINALS :", [f["nom"] for f in fertilisants])
...
def calcul_strict(window, Nb, Pb, Kb, ferts):
    ...
    debug.debug("Besoins :", Nb, Pb, Kb)
    debug.debug("Fertilisants :", ferts)

    if len(ferts) < 3:
        debug.debug("⛔ Strict impossible (<3 fertilisants)")
        QMessageBox.warning(
            window, "Erreur",
    ...
    debug.debug("Matrice A :", A)
    debug.debug("Vecteur B :", B)

    doses, *_ = np.linalg.lstsq(A, B, rcond=None)
    debug.debug("Doses brutes :", doses)

    resultats = []
    for dose, fert in zip(doses, ferts):
        dose = max(dose, 0)
        debug.debug(f"{fert['nom']} → dose corrigée = {dose}")
...
def calculer_doses_surface(window, resultats, culture):
    ...
    debug.debug(f"Surface culture = {surface} m²")
    ...
    for r in resultats:
        ...
        if not fert:
            debug.debug(f"⚠️ Fertilisant {r['nom']} introuvable dans fert_base")
            continue
        ...
        debug.debug(f"{r['nom']} → dose_surface={dose_surface:.2f} {unite}, prix_dose={prix_dose:.2f}€, quantite={quantite}, prix_ht={prix_ht:.2f}€")
    ...
    row = window.table_doses_surface.rowCount()
    window.table_doses_surface.insertRow(row)
    debug.debug(f"Ligne TOTAL → total_dose={total_dose:.2f}€, total_prix={total_prix:.2f}€")
