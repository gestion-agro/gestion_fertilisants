...
def menu_context_culture(window, pos):
    debug.debug("=== menu_context_culture ===")

    row = window.table_cultures.currentRow()
    debug.debug("Position clic :", pos, "| Ligne sélectionnée :", row)

    if row < 0:
        debug.debug("⛔ Aucune ligne sélectionnée → menu annulé")
        return
...
def menu_context_fert_droite(window, pos):
    debug.debug("=== menu context_fert_droite ===")
    ...
    if not window.culture_active:
        ...
        debug.debug("Action 'Utiliser' désactivée (pas de culture active)")
    ...
    debug.debug("Action sélectionnée :", action.text() if action else None)
    debug.debug("Fertilisant concerné :", nom)

    if action == action_utiliser:
        debug.debug("→ Utiliser fertilisant :", nom)
        ajouter_fert_utiliser(window,nom)

    elif action == action_modifier:
        debug.debug("→ Modifier fertilisant :", nom)
        modifier_fert(window, nom)

    elif action == action_supprimer:
        debug.debug("→ Supprimer fertilisant :", nom)
        supprimer_fert(window, nom)

    elif action == action_ajouter:
        debug.debug("→ Ajouter fertilisant")
        ajout_fert(window)
# ----------------------
...
def menu_context_fert_milieu(window, pos):
    debug.debug("=== menu_context_fert_milieu ===")
    ...
    if action == action_enlever:
        ...
        debug.debug("→ Enlever fertilisant :", nom)
