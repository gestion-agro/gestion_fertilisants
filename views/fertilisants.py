...
def ajout_fert(window, fert=None, clavier=False):
    debug.debug("=== ajout_fert ===")

    if clavier:
        debug.debug("Depuis raccourcis clavier")
    else:
        debug.debug("Depuis bouton")
        
    debug.debug("Fertilisant passée :", fert)
    ...
    debug.debug("Signal fertilisant_ajoute connecté → recharger_fertilisants")

    window.ajout_window.show()
    debug.debug("Fenêtre AjouterFertilisantWindow affichée")
# ----------------------

# ----------------------
# Modifier un fertilisant
def modifier_fert(window, nom):
    debug.debug("=== modifier_fert ===")
    ...
    debug.debug("Ouverture fenêtre modification fertilisant")
...
def double_clic_fertilisant(window, row, column):
    ...
    if not item:
        debug.debug(f"⚠️ Double-clic fertilisant invalide sur la ligne {row}")
        return
    ...
    if not window.culture_active:
        ...
        debug.debug(f"⚠️ Tentative d'ajout de fertilisant '{nom_fert}' sans culture active")
        return
    
    debug.debug(f"Double-clic fertilisant '{nom_fert}' → ajout à la culture '{window.culture_active}'")
